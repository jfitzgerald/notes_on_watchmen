      ...
      foreach my $ch (@{$book->{chapters}}) {
          ...
          foreach my $pg (@{$ch->{pages}}) {
              ...
              if(defined $color_data->{$page_id}) {
                  ...
                  $page_data->{page_num} = $pg->{page};

                  $page_data->{title} = sprintf("Chapter %d, Page %d", $ch->{chapter_num}, $pg->{page});
                  $page_data->{weight} = sprintf("%d%02d", $ch->{chapter_num}, $pg->{page});

                  my $pg_fname = sprintf("page-%02d-%02d.md", $ch->{chapter_num}, $pg->{page});
